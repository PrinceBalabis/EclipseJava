...
public class TestOrdinaryPlayer {
	public static void roll(Player player, int antalKast) {
		...
		// Använda OrdinaryPlayer-referensen för att erhålla spelarens
		// Dice-objekt
		// Här går det inte att använda Player-referensen (player) eftersom
		// metoden
		// getDice() inte är deklarerad i klassen Player (eller i Object).
		Dice dice = op.getDice();
		int sides = dice.getSides(), aThrow;
		// Skapa en array för att räkna antalet 1:or, 2:or ocv som tärningen
		// visar vid kasten nedan
		int[] result = new int[sides];
		TextWindow.println("----- " + antalKast + " kast av "
				+ player.getName() + " med " + sides + "-sidig tärning -----");
